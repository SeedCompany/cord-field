import { useQuery } from '@apollo/client';
import { Edit } from '@mui/icons-material';
import { Box, Skeleton, Typography } from '@mui/material';
import { Helmet } from 'react-helmet-async';
import { useParams } from 'react-router-dom';
import { canEditAny } from '~/common';
import { DetailProperty } from '~/components/DetailProperty';
import { useDialog } from '~/components/Dialog';
import { Error } from '../../../components/Error';
import { Fab } from '../../../components/Fab';
import { FormattedDateTime } from '../../../components/Formatters';
import { Redacted } from '../../../components/Redacted';
import { EditFieldZone } from '../Edit/EditFieldZone';
import { FieldZoneDocument } from './FieldZoneDetail.graphql';

export const FieldZoneDetail = () => {
  const { fieldZoneId = '' } = useParams();

  const [editZoneState, editZone] = useDialog();

  const { data, error } = useQuery(FieldZoneDocument, {
    variables: { fieldZoneId },
  });

  const fieldZone = data?.fieldZone;

  return (
    <Box
      component="main"
      sx={{
        flex: 1,
        overflowY: 'auto',
        p: 4,
      }}
    >
      <Helmet title={fieldZone?.name.value || undefined} />
      <Error error={error}>
        {{
          NotFound: 'Could not find field zone',
          Default: 'Error loading field zone',
        }}
      </Error>
      {!error && (
        <Box
          sx={{
            maxWidth: (theme) => theme.breakpoints.values.md,
            display: 'flex',
            flexDirection: 'column',
            gap: 3,
          }}
        >
          <Box
            component="header"
            sx={{
              flex: 1,
              display: 'flex',
            }}
          >
            <Typography
              variant="h2"
              sx={{
                mr: 4,
                width: !fieldZone?.name ? '40%' : undefined,
              }}
            >
              {!fieldZone ? (
                <Skeleton width="100%" />
              ) : (
                fieldZone.name.value ?? (
                  <Redacted
                    info="You don't have permission to view this field zone's name"
                    width="40%"
                  />
                )
              )}
            </Typography>
            {canEditAny(fieldZone, true) && (
              <Fab
                color="primary"
                aria-label="edit zone"
                onClick={editZone}
                loading={!fieldZone}
              >
                <Edit />
              </Fab>
            )}
          </Box>
          <Box
            sx={{
              display: 'flex',
              alignItems: 'baseline',
              gap: 2,
            }}
          >
            <Typography variant="h4">
              {fieldZone ? 'Field Zone' : <Skeleton width={200} />}
            </Typography>
            {fieldZone && (
              <Typography variant="body2" color="textSecondary">
                Created <FormattedDateTime date={fieldZone.createdAt} />
              </Typography>
            )}
          </Box>
          <DetailProperty
            label="Director"
            value={fieldZone?.director.value?.fullName}
            loading={!fieldZone}
          />
        </Box>
      )}
      <EditFieldZone fieldZone={fieldZone} {...editZoneState} />
    </Box>
  );
};
