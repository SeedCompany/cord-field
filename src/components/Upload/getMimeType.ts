...
// ai design-alignment Matches file-type's own internal minimum for reliable
// signature detection.
const FILE_TYPE_SNIFF_BYTES = 4100;

// ai design-alignment Signature for OLE Compound File Binary Format (.msg,
// legacy .doc/.xls/.ppt).
const OLE_SIGNATURE = [0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1];

// ai performance Bounds how much memory the MsgReader check below can use.
// Real .msg/.doc/.xls/.ppt files are well under this; anything bigger skips
// straight to the file-type sniff (which still resolves OLE files as
// `application/x-cfb`).
...
export const getMimeType = async (file: File) => {
  ...
  // ai edge-case MsgReader needs the whole buffer to walk the compound
  // file's directory structure - it can't work off a slice - so we gate it
  // on size + signature.
