...
/**
 * Nothing uses this config yet — Razzle is still the build. It exists so
 * `vite build` output can be compared against Razzle's by hand before
 * `stage-03-issue-02` (the dev server) and `stage-03-issue-03` (the production
 * build) wire it up.
 */
export default defineConfig(({ isSsrBuild, mode }) => {
  ...
  return {
    plugins: [
      ...appPlugins(),
      // Client only. webpack 4 auto-polyfilled these; Vite polyfills nothing,
